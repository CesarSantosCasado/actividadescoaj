  ...
  try {
    ...
    if (fechaNacimiento) {
      ...
      if (!isNaN(nac)) {
        ...
        if (edad < 14) return res.json({ success: false, message: 'Debes tener al menos 14 años para inscribirte.' });
        if (edad > 30) return res.json({ success: false, message: 'COAJ está dirigido a jóvenes de 14 a 30 años.' });
      }
    }
    ...
    if (check.yaInscrito) return res.json({ success: false, message: 'Ya estás inscrito en esta actividad.' });
...
app.post('/api/reservar', async (req, res) => {
  ...
  try {
    ...
    const horaFin = sumarHora(horaReserva, 2);
    ...
    if (conflicto) return res.json({ success: false, message: 'Horario no disponible' });
...
app.post('/api/actualizar-foto', async (req, res) => {
  ...
  if (!alias || !fotoUrl) return res.json({ success: false, message: 'Datos incompletos' });
